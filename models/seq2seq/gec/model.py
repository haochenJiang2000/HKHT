...
from supar.modules.transformer import TransformerDecoder, TransformerDecoderLayer
...
class Seq2SeqModel(Model):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.args.encoder == 'transformer':
            ...
            self.decoder = TransformerDecoder(
                layer=TransformerDecoderLayer(
                    n_heads=self.args.n_decoder_heads, n_model=self.args.n_decoder_hidden, n_inner=self.args.n_decoder_inner, dropout=self.args.decoder_dropout
                ),
            ...
            self.model = AutoModel.from_pretrained(self.args.bart, dropout=self.args.dropout)
        ...
        self.classifier.weight = (self.word_embed.embed if self.args.encoder == 'transformer' else self.model.shared).weight
        self.criterion = CrossEntropyLoss(label_smoothing=self.args.label_smoothing)
    ...
    def forward(self, words):
        ...
        if self.args.encoder == 'transformer':
            ...
            embed = embed * self.word_embed.embed_scale + self.word_embed.pos_embed(embed)
            embed = self.embed_dropout(embed)
            return self.encoder(embed, words.ne(self.args.pad_index))
        else:
            return self.encoder(input_ids=words, attention_mask=words.ne(self.args.pad_index))[0]
    ...
    def loss(self, x, tgt, src_mask, tgt_mask):
        if self.args.encoder == 'transformer':
            ...
            _, seq_len = tgt.shape
            attn_mask = tgt.new_ones(seq_len, seq_len, dtype=torch.bool).tril_()
            y = self.decoder(self.embed(shifted), x, tgt_mask, src_mask, attn_mask)
        else:
    ...
    def mr_loss(self, x, tgt, src_mask, tgt_mask, ref_nums, epoch):
        ...
        total_loss = 0
        if epoch == 1:
            for sample_index, ref_num in enumerate(ref_nums):
                avg_loss = 0
    ...
    def mr_loss_test(self, x, tgt, src_mask, tgt_mask, ref_nums):
        assert self.args.encoder == 'bart'
        x = torch.cat([torch.stack([x[i]] * ref_nums[i], 0) for i in range(len(ref_nums))], 0)
        src_mask = torch.cat([torch.stack([src_mask[i]] * ref_nums[i], 0) for i in range(len(ref_nums))], 0)
        shifted = torch.full_like(tgt, self.args.eos_index)
    ...
    def decode(self, x, src_mask):
        ...
        tgt = x.new_full((batch_size * beam_size, 1), self.args.bos_index, dtype=torch.long)
        # [batch_size * beam_size]
        active = src_mask.new_ones(batch_size * beam_size)
        # [batch_size]
        batches = tgt.new_tensor(range(batch_size)) * beam_size
        # accumulated scores
        scores = x.new_full((batch_size, self.args.beam_size), MIN).index_fill_(-1, tgt.new_tensor(0), 0).view(-1)

        def rank(scores, mask, k):
            scores = scores / mask.sum(-1).unsqueeze(-1) ** self.args.length_penalty
        ...
        for t in range(1, min(self.args.max_len + 1, int(1.8 * x.shape[1]))):
            ...
            if self.args.encoder == 'transformer':
                ...
                s_y = self.decoder(self.embed(tgt[active]), x[active], tgt_mask[active], src_mask[active], attn_mask)
                # [n_active, n_words]
                ...
                s_y, new_past_key_values = self.decoder(
                    input_ids=input_ids,
                    attention_mask=torch.cat((torch.ones_like(tgt_mask[:, :1]), tgt_mask), 1),
                    encoder_hidden_states=x,
                    encoder_attention_mask=src_mask,
            ...
            active = tokens.ne(tokens.new_tensor((self.args.eos_index, self.args.pad_index))).all(-1)

            if not active.any():
                break
        cands = rank(scores.view(-1, 1), tgt.ne(self.args.pad_index), self.args.topk)

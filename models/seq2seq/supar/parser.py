...
class Parser(object):
    ...
    def train(
        ...
        **kwargs,
        ...
        train = Dataset(self.transform, args.train, **args).build(
            batch_size=batch_size, n_buckets=buckets, shuffle=False, distributed=is_dist(), even=args.even, seed=args.seed, n_workers=workers
        )
        dev = Dataset(self.transform, args.dev, **args).build(
            batch_size=eval_batch_size, n_buckets=buckets, shuffle=True, distributed=is_dist(), even=False, n_workers=workers
        )
        ...
        if not args.test:
            ...
            test = Dataset(self.transform, args.test, **args).build(
                batch_size=eval_batch_size, n_buckets=buckets, shuffle=False, distributed=is_dist(), even=False, n_workers=workers
            )
            logger.info(f"{'dev:':6} {dev}")
        ...
        if args.wandb and is_master():
            ...
            wandb.init(config=args.primitive_config, project=args.get('project', self.NAME), name=args.get('name', args.path), resume=self.args.checkpoint)
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        data.build(batch_size=batch_size, n_buckets=buckets, shuffle=False, distributed=is_dist(), even=False, n_workers=workers)
        logger.info(f"\n{data}")
        ...
        logger.info(f"{elapsed}s elapsed, " f"{sum(data.sizes)/elapsed.total_seconds():.2f} Tokens/s, " f"{len(data)/elapsed.total_seconds():.2f} Sents/s")
    ...
    def predict(
        ...
        **kwargs,
        ...
        start = datetime.now()
        self.model.eval()
        with tempfile.TemporaryDirectory(dir="/data/hcjiang/tmp") as t:
    ...
    def clip_grad_value_(self, params: Union[Iterable[torch.Tensor], torch.Tensor], clip_value: float) -> None:
        self.scaler.unscale_(self.optimizer)
    ...
    def init_scheduler(self) -> _LRScheduler:
        if self.args.encoder == 'lstm':
            scheduler = ExponentialLR(optimizer=self.optimizer, gamma=self.args.decay ** (1 / self.args.decay_steps))
        elif self.args.encoder == 'transformer':
            scheduler = InverseSquareRootLR(optimizer=self.optimizer, warmup_steps=self.args.warmup_steps)
        else:
            scheduler = LinearLR(
                optimizer=self.optimizer, warmup_steps=self.args.get('warmup_steps', int(self.args.steps * self.args.get('warmup', 0))), steps=self.args.steps
            )
        return scheduler
    ...
    def save(self, path: str) -> None:
        ...
        state = {'name': self.NAME, 'args': model.args, 'state_dict': state_dict, 'pretrained': pretrained, 'transform': self.transform}
